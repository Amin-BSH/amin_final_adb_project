...
class FactorySugar(Base, TimestampMixin):
    __tablename__ = "factory_sugar"

    id: Mapped[int] = mapped_column(BigInteger, primary_key=True)
    amount: Mapped[float] = mapped_column(Float, nullable=True)
    sugar_weight_received_factory: Mapped[float] = mapped_column(Float, nullable=True)
    sugar_price_received_factory: Mapped[float] = mapped_column(Float, nullable=True)
